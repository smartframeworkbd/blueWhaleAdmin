...
const HotSpot = () => {
  ...
  ?.filter(hotspot => hotspot?.hotspotSectionName === "sectionOne")
  ?.map((hotspot, index) => ({
    key: index + 1,
    id: hotspot.id,
  ...
  const handleEditHotspot = (hotspotId) => {
    router.push(`/Dashboard/HotSpot/EditHotSpot?id=${hotspotId}`);
  };
    ...
    <>
       <div>
        <p className="lg:text-2xl text-center font-bold mb-10">
          {currentLanguage === "en" ? "Manage your hotspot easily from here" : "এখান থেকে সহজেই আপনার হটস্পট ওয়ান পরিচালনা করুন"}
        </p>
      </div>
      <div className="flex flex-col lg:flex-row items-center gap-x-2 justify-end my-5">
        <ButtonWithModal title={translations[currentLanguage].addHotspot} path={'/Dashboard/HotSpot/AddHotSpot'}/>
      </div>
